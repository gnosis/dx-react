...
module.exports = async () => {
  if (!(argv.eth > 0 || argv.gno > 0 || argv.tul > 0 || argv.owl > 0) || !(argv.master || argv.seller || argv.buyer || argv.a)) {
  ...
  const account = argv.master ? master : argv.seller ? seller : argv.buyer ? buyer : argv.a
  const accountName = argv.master ? 'Master' : argv.seller ? 'Seller' : argv.buyer ? 'Buyer' : `Acc ${argv.a}`
  ...
  await depositToDX(account, tokensToGive);
