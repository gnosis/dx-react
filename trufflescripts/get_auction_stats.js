...
module.exports = async () => {
  ...
  const sellerBalanceNext = sellVolumeNext &&
    (await dx.sellerBalances(eth.address, gno.address, auctionIndex + 1, seller)).toNumber()
  ...
  const readStats = async (i) => {
    const buyVolume = (await dx.buyVolumes(eth.address, gno.address, i)).toNumber()

    let price, amountToClearAuction, timeUntilAuctionClears
    try {
      const [num, den] = (await dx.getPrice(eth.address, gno.address, i)).map(n => n.toNumber())
    ...
    const sellerBalance = (await dx.sellerBalances(eth.address, gno.address, i, seller)).toNumber()
    const buyerBalance = (await dx.buyerBalances(eth.address, gno.address, i, buyer)).toNumber()
