...
contract('DutchExchange', (accounts) => {
  ...
  it('check fee calculation for sellOrders orders with zero tulip', async () => {
    ...
    let calculatedFee = 10 ** 9 * 0.005
    await checkBalanceBeforeClaim(buyer1, auctionIndex, 'buyer', eth, gno, (10 ** 9 - calculatedFee))
    calculatedFee = 10 ** 9 * 2 * 0.005
    await checkBalanceBeforeClaim(seller1, auctionIndex, 'seller', eth, gno, (10 ** 9 * 2 - calculatedFee))
  })
})
