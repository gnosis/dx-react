...
async function setupTest(accounts) {
  ...
  await Promise.all(accounts.map((acct) => {
    ...
    gno.transfer(acct, 10 * ONE, { from: accounts[0] })
    gno.approve(dx.address, 10 * ONE, { from: acct })
  }))
  ...
  await Promise.all(accounts.map((acct) => {
    ...
    dx.deposit(eth.address, 10 * ONE, { from: acct })
    dx.deposit(gno.address, 10 * ONE, { from: acct })
  }))
